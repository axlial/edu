...
ls -1 /var/lab/3/ | sort -n > /var/lab/3/6
...
free -k | sed -n 2p | awk '{ print $2 }' >> /var/lab/3/15
fdisk -l | grep Disk | sed -n 1p | awk '{ print $5 }' >> /var/lab/3/15

Задание 3.16 Задание с inodes
Для выполнения задания воспользуйтесь утилитой df. 
df -i /dev/mapper/centos-root | sed -n 2p | awk '{ print $2 }' > /var/lab/3/16
...
Для выполнения задания отмонтируйте /var/lab/3/18, удалите его и выведите содержимое файла /proc/filesystems, отсортируйте его и перенаправьте вывод в файл.
umount /var/lab/3/18; rm -rf /var/lab/3/18; cat /proc/filesystems | cut -f 2 | sort -s | tee /var/lab/3/18
...
chown 167:168 /var/lab/4/1
chmod 251 /var/lab/4/1
...
ln -s /var/lab/5/2file /var/lab/5/2link && chown -h user12:root /var/lab/5/2link
...
zcat /var/lab/8/1.gz > /var/lab/8/1.1
...
tar -xzvf /var/lab/8/5.tar.gz -C /var/lab/8/ lab/smallfile
mv /var/lab/8/lab/smallfile /var/lab/8/15
...
bunzip2 /var/lab/8/7.bz2 | xz > /var/lab/8/7.xz
...
Задание 11.1 Шлюз внешнего адаптера
Для выполнения задания воспользуйтесь информацией из конфигурационного файла /etc/sysconfig/network-scripts/ifcfg-ens192.
echo "185.66.68.1" > /var/lab/11/1

Задание 11.2 IP внешнего адаптера
Для выполнения задания воспользуйтесь информацией из конфигурационного файла /etc/sysconfig/network-scripts/ifcfg-ens192.
echo "185.66.68.62" > /var/lab/11/2

Задание 11.3 Маска внешнего адаптера
Для выполнения задания воспользуйтесь информацией из конфигурационного файла /etc/sysconfig/network-scripts/ifcfg-ens192.
echo "255.255.255.128" > /var/lab/11/3

Задание 11.4 Мак-адрес внешнего адаптера
Для выполнения задания воспользуйтесь утилитой IP.
ip a
echo "02:00:b9:42:44:3e" > /var/lab/11/4

Задание 11.5 Названия интерфейсов ### Ошибка код 10!!!
Для выполнения задания воспользуйтесь утилитой IP.
ip a| grep state| awk {'print$2'}| cut -d: -f1| sort | tee /var/lab/11/5
 
Задание 11.6 Настройка работы сети ### Задание не понял, плейбук проверяет вывод w и сравнивает его со строкой up. Действий никаких не требуется. Скорее всего нужно тушить интрефейс и предоставлять доступ пользователю через VNC. 
Для выполнения задания воспользуйтесь команой ip.

Задание 11.7 Локальный интерфейс
Для выполнения задания воспользуйтесь утилитой ip.
ip a add 192.168.11.7/32 dev lo:1

Задание 11.8 Форвардинг пакетов
Для выполнения задания отредактируйте файл /etc/sysctl.conf
vi /etc/sysctl.conf
net.ipv4.ip_forward = 1
sysctl -p

Задание 12.1 Список портов
Для выполнения задания воспользуйтесь утилитами netstat, grep, awk, cut, sort и uniq. Перенаправьте вывод в файл.
netstat -plnt|grep tcp|awk '{print $4}'|rev|cut '-d:' -f1|rev|sort -rn|uniq | tee /var/lab/12/1

Задание 12.2 Доступные порты
Для выполнения задания воспользуйтесь утилитами netstat, grep, awk, cut, sort и uniq. Перенаправьте вывод в файл.
netstat -plnt|grep tcp| awk '{print $7}'| cut -d/ -f2|sort|uniq|sort | tee /var/lab/12/2

Задание 13.1 Основные записи домена
Для выполнения задания воспользуйтесь утилитой dig. Перенаправьте вывод в файл. Удалите в файле кавычки в записях TXT.
dig +short support.by > /var/lab/13/1 && dig +short support.by mx | awk '{print $2}' >> /var/lab/13/1 && dig +short support.by ns | sort >> /var/lab/13/1 && dig +short support.by soa >> /var/lab/13/1 && dig +short support.by txt >> /var/lab/13/1

Задание 13.2 Время жизни записи
Для выполнения задания воспользуйтесь утилитой dig. Перенаправьте вывод в файл.
dig @dns1.helpdesk.by my.support.by | grep 77.72.128.92 | awk '{print $2}' | tee /var/lab/13/2

Задание 13.3 Обратная запись
Для выполенния задания воспользуйетсь утилитой dig. Перенаправьте вывод в файл.
dig -x 185.66.69.200 +short | tee /var/lab/13/3


Замечания при прохождении курса.

Нужно ОБЯЗАТЕЛЬНО заменить ответы в подсказаках Телеграмм-бота - 3.17, 3.18, 3.19, 4.1, 4.4, 4.6, 5.2, 8.5
Проверить плейбуки заданий 4.4, 8.7. Ответы выше в файле. 
Нужно ли задание 11.6?
